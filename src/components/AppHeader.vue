<template>
  <div class="navbar bg-base-100">
    ...
    <div class="navbar-start">
      ...
      <router-link :to="{ name: 'home' }" class="btn btn-ghost normal-case text-xl inline-flex items-center gap-4">
    ...
    <div v-else class="flex items-center justify-end navbar-end space-x-2 sm:space-x-4">
      ...
      <div class="dropdown dropdown-end">
        <label tabindex="0" class="btn m-1 p-3" style="display: flex; flex-direction: column; align-items: center">
          <font-awesome-icon :icon="['fas', 'cart-shopping']" />
        </label>
        <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-96">
          <li>
            <router-link :to="{ name: 'cart' }" style="display: flex; justify-content: space-between">
              <span>Lobster</span>
              <img src="@/assets/images/cart/lobster.webp" width="100" height="100" style="float: right" />
            </router-link>
          </li>
          <li>
            <router-link :to="{ name: 'cart' }" style="display: flex; justify-content: space-between">
      ...
      <!-- Avatar -->
      <a class="block shrink-0 pr-10">
        <span class="sr-only">Profile</span>
        <img
          src="https://images.unsplash.com/photo-1600486913747-55e5470d6f40?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80"
          class="h-10 w-10 rounded-full object-cover hover:cursor-pointer" @click="showSidebar = !showSidebar" />
      </a>
    </div>
  </div>
  ...
  <div>
    <div v-if="showSidebar" class="flex h-screen flex-col justify-between bg-white">
      <div class="px-4">
        <span class="mt-5 mr-5 flex justify-end hover:cursor-pointer" @click="showSidebar = false"><font-awesome-icon
            :icon="['fas', 'xmark']" /></span>
